...
左上角侧边栏那一行——装着 logo 的 `logoRow`——里有一个外壳注入的 **更新引擎** 按钮，悬停显示当前引擎版本与安装位置。它装在**引擎自己返回的那个页面里**（`Builder::on_page_load` 在每次页面加载完成后 eval 注入），并且直接插进那一行的 flex 流：不覆盖任何东西，也不挤压布局。React 重渲染会丢掉外来节点，所以轮询每次都检查按钮还在不在，掉了就重新插回去。进度与结果用一个**按该行自身坐标定位**的浮层显示，不参与那一行的布局。

点按钮后：查镜像的发布通道 → 比对版本 → 用 npm 更新 → **重启外壳自己启动的那个引擎**（杀掉并重新拉起，窗口落到新引擎上）。npm 不提供百分比，所以进度按"哪个信号在动"推断，显示 `正在获取包元数据… / 正在下载依赖包… / 正在解压安装…` 加已下载字节、网速、已安装包数。
