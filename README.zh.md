...
窗口顶部有一条外壳注入的工具条，显示当前引擎版本（`dsh x.y.z`）和一个**更新引擎**按钮。它装在**引擎自己返回的那个页面里**（`Builder::on_page_load` 在每次页面加载完成后 eval 注入），而且是把页面**下推** 32px 而不是浮在上面覆盖——界面的高度是一条从 `body` 起的 `height:100%` 链（`#root{height:100%}`），缩 `body` 不会遮住应用自己的顶栏。
