...
4. **The close button (×) hides the shell to the tray instead of ending it** — the window is the surface, not the application. When the shell spawned its own host, ending the process ends that engine, and a browser tab pointed at the same engine dies with it. So × only hides the window and the engine keeps serving behind the tray; **only the tray menu's quit item really ends the process**, stopping the host the shell owns. A service the shell merely attached to or reused is not owned and survives either way. Left-click the tray icon to get the window back, right-click for the menu. A shell whose window no longer exists (an abnormal case) exits instead of lingering without a surface.
5. On exit, the host the shell spawned is killed (`Drop for DesktopState` is the ordinary path; `RunEvent::Exit` is the fallback that keeps an exit skipping the drop from orphaning it).

## Updating the engine (top-left of the window's title bar)

The window is **undecorated** (`decorations: false`) and its caption is drawn by the injected script: app icon and the **update engine** button on the left, minimize / maximize / close on the right, with drag on the strip and double-click to maximize. Windows cannot host webview content in a native caption — `titleBarStyle` is macOS-only and neither Tauri nor wry exposes the WebView2 title-bar overlay — so drawing the whole bar is the only way to put a control at the window's top-left.

The script is injected into **every page the window shows** (`Builder::on_page_load`): the engine's own page and the splash, because an undecorated window could otherwise be neither moved nor closed while it loads. Only the engine page gets the engine half. The page is offset by the bar's 32px so nothing is hidden behind it.

The window actions go through the **shell's own commands** (`window_drag`, `window_minimize`, `window_toggle_maximize`, `window_close`) rather than granting `core:window:*` to the engine page: adding one command to the manifest is a far narrower grant than handing a remote origin the window plugin's whole permission surface. Closing still takes the native path — hide to the tray, with only the tray's quit item ending the process.

Pressing the button resolves the mirror's release channels, compares versions, updates with npm, and **restarts the engine the shell itself started** (kill and respawn, with the window landing on the new one). npm reports no percentage, so progress is inferred from which signal is moving — `正在获取包元数据… / 正在下载依赖包… / 正在解压安装…` plus bytes, speed and installed package count, shown in a popover under the button.

**The cost**: some native title-bar integration is gone — Windows 11's Snap Layouts flyout no longer appears when hovering the maximize button (`Win`+arrow and dragging to an edge still snap), and the bar no longer follows the system theme.

Pressing it resolves the mirror's release channels, compares versions, updates with npm, and **restarts the engine the shell itself started** (kill and respawn, with the window landing on the new one). npm reports no percentage, so progress is inferred from which signal is moving — `正在获取包元数据… / 正在下载依赖包… / 正在解压安装…` plus bytes, speed and installed package count.

Two deliberate choices:

- **It does not install `@latest`.** This package keeps `latest` on an *older* RC than `next` (measured `latest = 0.1.5-rc.1`, `next = 0.1.5-rc.2`), so `@latest` would **downgrade** a current engine. The shell reads `dist-tags`, takes the higher semver of `latest` and `next` (falling back to every tag when neither exists), and only acts when it is strictly newer — otherwise it reports "already current" without touching a single file.
- **It only restarts an engine it owns.** An attached service is not the shell's (see above); killing it would kill the process you started in a terminal. That case reports "the engine was started by another process, restart it to apply" instead. To let the desktop app take over, don't start an engine yourself first.

**Security boundary**: the engine's page is `http://127.0.0.1:…`, a *remote* origin to Tauri, and gets no IPC by default. `capabilities/engine-surface.json` grants it exactly three shell commands (`allow-engine-status`, `allow-engine-update`, `allow-toolbar-ready`) and **no core or plugin permission at all**; those `allow-*` identifiers are generated by `AppManifest::commands` in `build.rs`. The local splash keeps the `default` capability (`core:default` + `allow-startup-status`).
