...
## Updating the engine (the button beside the logo)

An **update engine** button sits in the sidebar's brand row — `logoRow`, the strip holding the logo — and its tooltip carries the installed version and location. It lives inside **the page the engine itself serves** (injected by `Builder::on_page_load` after every document load) and goes straight into that row's flex flow: nothing is overlaid and nothing is pushed around. React drops foreign nodes when it re-renders, so every poll checks that the button is still there and re-attaches it if it is not. Progress and the result appear in a popover positioned from the row's own rectangle, so they never disturb the layout they report on.
