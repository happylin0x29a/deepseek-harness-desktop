...
## Updating the engine (the toolbar at the top of the window)

A shell-injected toolbar sits at the top of the window with the current engine version (`dsh x.y.z`) and an **update engine** button. It lives inside **the page the engine itself serves** (injected by `Builder::on_page_load` after every document load) and **pushes the page down** by 32px instead of floating over it — the surface is a `height: 100%` chain from `body` (`#root{height:100%}`), so shrinking `body` cannot cover the app's own top row.
